...
def ambil_data_saham(kode, waktu_mulai, waktu_akhir):
    import os
    # Mengutamakan data lokal agar evaluasi model konsisten
    ...
    try:
        # Mendownload data dari API yfinance
        ...
        # Menggunakan data cadangan CSV dari GitHub
        st.warning("Menggunakan data cadangan CSV dari GitHub. Koneksi Yahoo Finance limit/error.")
        try:
...
# Membulatkan harga penutupan ke 2 desimal agar data input stabil & konsisten
if not data_saham.empty:
    for col in ['Open', 'High', 'Low', 'Close']:
        if col in data_saham.columns:
            data_saham[col] = data_saham[col].round(2)
...
st.info("Evaluasi model menggunakan 80% data training dan 20% data uji untuk memastikan akurasi yang valid.")
...
with tab_prophet:
    ...
    st.subheader("Evaluasi Akurasi Model Prophet (Data Test 20%)")
    ...
    with st.expander("Dekomposisi Komponen Prophet (Tren & Musiman)"):
        st.write("Grafik ini membedah prediksi Prophet menjadi komponen pembentuknya: Tren jangka panjang, pola mingguan, dan pola tahunan yang dimana membuktikan bahwa model mampu menangkap fluktuasi pasar.")
        st.plotly_chart(grafik_komponen, use_container_width=True)
    
    # Tabel Pembuktian
    with st.expander("Tabel Pembuktian"):
        ...
        cols_p = ['Harga Asli', 'Prediksi Prophet', 'Selisih (Error)', 'Error Mutlak (MAE)', 'Error Kuadrat (RMSE)', 'Persentase Error (%)']
        df_prov_prophet[cols_p] = df_prov_prophet[cols_p].round(2)
...
with tab_lstm:
    ...
    # Kurva Pelatihan Model
    with st.expander("Kurva Pelatihan Model (Loss vs Epoch)"):
